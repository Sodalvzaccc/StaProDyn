...
from models.StaProDynModal import build_model
...
def train(model, train_loader, optimizer, loss_fn, epoch, metrics):
    ...
    for data in train_pbar:
        ...
        weak_mode = torch.zeros(inputs['V'].size(0), dtype=torch.long, device=device)
        output, nce_loss = model(inputs, copy_label, weak_mode, epoch=epoch, opt=opt)
...
def evaluate(model, eval_loader, optimizer, loss_fn, epoch, metrics):
    ...
    with torch.no_grad():
        for data in test_pbar:
            ...
            weak_mode = torch.zeros(inputs['V'].size(0), dtype=torch.long, device=device)
            output, _ = model(inputs, None, weak_mode)
            y_pred.append(output.cpu())
            y_true.append(label.cpu())
