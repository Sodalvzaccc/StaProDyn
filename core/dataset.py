...
class MMDataset(Dataset):
    def __init__(self, args, mode='train', drop_rate=0.6, full_data=False):
        self.mode = mode
        self.args = args
    ...
    def __init_mosi(self):
        ...
        if 'sims' in self.args.datasetName:
            for m in "TAV":
                self.labels[m] = data[self.mode][self.args.train_mode + '_labels']
    ...
    def __gen_mask(self, data):
        ...
        self.vision_padding_mask = vision_mask.to(torch.bool)
        ...
        self.audio_padding_mask = audio_mask.to(torch.bool)
    ...
    def __getitem__(self, index):
        sample = {
            ...
            'missing_mode': self.get_missing_mode()
        }
...
def MMDataLoader(args):
    datasets = {
        'train': MMDataset(args, mode='train', drop_rate=0.6, full_data=False),
        'valid': MMDataset(args, mode='valid', drop_rate=0.6, full_data=True),
        'test': MMDataset(args, mode='test', drop_rate=0.6, full_data=True)
    }
