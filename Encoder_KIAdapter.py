...
from custom_transformer import MyTransformerEncoder
from torch.nn import TransformerEncoderLayer
from transformers import BertTokenizer, BertModel, BertConfig
import os
# 使用原始字符串方式防止路径转义错误
model_path = r"/root/autodl-tmp/3wd_drt/pretrainedModel/bert-base-uncased/"
...
class MixDomainAdapter(nn.Module):
    def __init__(self, up_prj, down_prj, nhead=4, num_layers=2, dropout=0.1):
        ...
        tfencoderlayer = TransformerEncoderLayer(down_prj, nhead, down_prj // 2, dropout=dropout, activation='gelu',
                                                 batch_first=True)
        self.tfencoder = MyTransformerEncoder(tfencoderlayer, num_layers)
    ...
    def forward(self, ex_know, src_key_padding_mask):
        ...
        hidden, _ = self.tfencoder(hidden, mask=None, src_key_padding_mask=src_key_padding_mask)
...
class TfEncoder(nn.Module):
    def __init__(self, fea_size, num_patches, nhead, dim_feedforward, num_layers, pos_dropout=0., tf_dropout=0.2):
        ...
        tfencoder_layer = TransformerEncoderLayer(dim_feedforward, nhead, dim_feedforward // 2, dropout=tf_dropout,
                                                  activation='gelu', batch_first=True)
        self.tfencoder = MyTransformerEncoder(tfencoder_layer, num_layers)
...
class UniEncoder(nn.Module):
    def __init__(self, m, pretrained, num_patches, fea_size, nhead, dim_feedforward, num_layers):
        ...
        if m in "VA":
            ...
            self.model_config = BertConfig.from_pretrained(model_path, output_hidden_states=True)
            self.tokenizer = BertTokenizer.from_pretrained(model_path, do_lower_case=True)
            self.model = BertModel.from_pretrained(model_path, config=self.model_config)
    ...
    def forward(self, inputs, key_padding_mask):
        if self.m in "VA":
            ...
            '''
            input_ids, input_mask, segment_ids = inputs[:, 0, :].long(), inputs[:, 1, :].float(), inputs[:, 2,
                                                                                                  :].long()  # 更换原始文本，使用tokenizer
            ...
            key_padding_mask = torch.tensor(key_padding_mask, dtype=torch.bool).cuda()
...
class UniPretrain(nn.Module):
    def __init__(self, modality, num_patches, pretrained='./pretrainedModel/bert-base-uncased', fea_size=709,
                 proj_fea_dim=128, drop_out=0.):
...
class UnimodalEncoder(nn.Module):
    def __init__(self, opt, bert_pretrained='./pretrainedModel/bert-base-uncased'):
        super(UnimodalEncoder, self).__init__()
        # All Encoders of Each Modality
        self.enc_t = UniPretrain(modality="T", pretrained=bert_pretrained, num_patches=opt.seq_lens[0],
                                 proj_fea_dim=768)
        self.enc_v = UniPretrain(modality="V", num_patches=opt.seq_lens[1], fea_size=20)
        self.enc_a = UniPretrain(modality="A", num_patches=opt.seq_lens[2], fea_size=5)
